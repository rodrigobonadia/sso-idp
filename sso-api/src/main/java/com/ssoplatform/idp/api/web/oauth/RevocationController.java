...
import com.ssoplatform.idp.application.usecase.revocation.RevokeTokenCommand;
...
/**
 * {@code POST /revoke}: the OAuth2 Token Revocation endpoint (RFC 7009), letting a client
 * proactively invalidate a refresh token it holds - see {@code RevokeTokenUseCase}'s Javadoc for
 * why only refresh tokens are genuinely revocable here, why the whole rotation family dies
 * together, and why every outcome about the TOKEN itself (unknown, already revoked, belonging to
 * another client) is a silent success rather than a distinguishing error.
 *
 ...
 * Security session. {@link OAuthRevocationException} is the only exception this controller
 ...
 *
 * <p>Responds {@code 200 OK} with an empty body for every request that could be evaluated at all,
 * per RFC 7009 §2.2 - whether a token was actually found and revoked or not.
 */
@RestController
public class RevocationController {
    ...
    public RevocationController(RevokeTokenUseCase revokeTokenUseCase, TenantContext tenantContext) {
    ...
    public ResponseEntity<?> revoke(
        ...
        try {
            revokeTokenUseCase.execute(command);
            return ResponseEntity.ok().build();
        } catch (OAuthRevocationException ex) {
            return errorResponse(ex);
        }
    }

    private static ResponseEntity<OAuthErrorResponse> errorResponse(OAuthRevocationException ex) {
